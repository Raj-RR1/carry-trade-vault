...
12. [EVM ↔ Substrate Address Conversion](#8-evm--substrate-address-conversion)
...
**Important:** After the Nov 2025 migration, Polkadot Hub (AssetHub) is the DOT reserve. `DepositReserveAsset` is how you send DOT to other chains. DOT on destination chains is identified as `{parents:1, X1(Parachain(1000))}`.
...
### 5. Post-Migration DOT Location

Since Nov 2025, AssetHub IS the DOT reserve. On destination chains, DOT is:
- **On AssetHub (local):** `{parents: 0, interior: Here}`
- **On siblings (Bifrost, Hydration, etc.):** `{parents: 1, interior: X1(Parachain(1000))}`

NOT `{parents: 1, interior: Here}` (that was the old relay location).
...
### 8. EVM ↔ Substrate Address Conversion
...
mod xcm_lib {
    ...
    pub fn build_transact(
        ...
        let message = Xcm::<()>(vec![
            WithdrawAsset((Parent, Parachain(1000), fee_amount).into()),
            BuyExecution {
                fees: (Parent, Parachain(1000), fee_amount).into(),
                weight_limit: Unlimited,
            },
            Transact {
                origin_kind: OriginKind::SovereignAccount,
...
| Approach | Pros | Cons |
|----------|------|------|
| **Pure Solidity** (our approach) | Single language, no dependencies, hackathon "PVM experiment" | Manual byte encoding, error-prone, verbose |
...
ink! v6 with PVM/`pallet_revive` support is in active development. The migration from Wasm (`pallet_contracts`) to RISC-V (`pallet_revive`) is underway. Check [use.ink](https://use.ink) for the latest on cross-contract calling between ink! and Solidity.

> **For this hackathon:** We went with pure Solidity SCALE encoding as the "PVM experiment" — proving that XCM can be composed entirely within Solidity on PolkaVM. But in production, an ink! XCM library would be the cleaner architecture.
